    ...
    : IManejadorDeConsulta<ObtenerEnlaceDescargaQuery, EnlaceDescargaResponse>
{
    ...
    /// Se lanza si el documento no existe para el solicitante, si su rol no puede
    /// descargarlo o si no tiene habilitada la acción de descarga.
    /// </exception>
        ...
        ObtenerEnlaceDescargaQuery consulta, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Los permisos personalizados cuentan: el administrador puede retirar la descarga a un usuario.
            _politicaDeAcceso.GarantizarPuedeEjecutar(
                _usuarioActual.Rol, _usuarioActual.Permisos, AccionDelSistema.DescargarDocumentos);
    ...
    /// <summary>Registra en la bitácora un intento de descarga rechazado.</summary>
